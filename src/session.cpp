...
void Session::begin(std::vector<std::string> front, std::vector<std::string> back,
	...
	for (auto i = front.begin(), z = back.begin(); i != front.end(); ++i, ++z)
	{
		...
		*/
		...
		while (c != '\n')
		{
			...
			if ((c == KEY_BACKSPACE || c == KEY_DC || c == 127 || c == 263) && input.size() != 0)
			{
				input.pop_back();
				printw("_");
				move(yCur, xCur);
			}
			else if ((c == KEY_BACKSPACE || c == KEY_DC || c == 127 || c == 263) && input.size() == 0)
			{
				move(yCur, xCur);
				printw(" ");
				refresh();
